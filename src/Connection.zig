...
pub const Request = struct {
    ...
    pub const Error = error{
        /// The request is already sent, no more edits can be done
        AlreadySent,
    } || std.Io.Writer.Error;

    pub fn writeHeader(self: *@This(), header: Header) Error!void {
    ...
    pub fn writeValue(self: @This(), value: anytype) Error!void {
        if (self.sequence != null) return error.AlreadySent;
        const endian = self.connection.endian;
        const writer = &self.connection.*.writer.interface;
        inline for (@typeInfo(@TypeOf(value)).@"struct".fields) |field| {
            ...
            switch (@typeInfo(field.type)) {
                ...
                .@"struct" => |s| switch (s.layout) {
                    .auto => try writeValue(self, field_val),
    ...
    pub fn updateHeader(self: @This()) !void {
        if (self.sequence != null) return error.AlreadySent;
        const header_index = self.header_index.?;
        const writer = &self.connection.*.writer.interface;
        if (self.header == null) return error.NoHeader;
        var length_buffer: [@divExact(@typeInfo(@typeInfo(Request.Length).@"enum".tag_type).int.bits, 8)]u8 = undefined;
        std.mem.writeInt(u16, &length_buffer, Length.fromBytes(writer.end - header_index).toWords(), self.connection.endian);
        writer.buffer[header_index + 2] = length_buffer[0];
        writer.buffer[header_index + 3] = length_buffer[1];
    }
    ...
    //     var request: @This() = .{ .connection = connection, .header = header, .start = start, .end = end, .sequence = connection.sequence };
    ...
    pub fn receiveReply(self: @This(), T: type) !Reply(T) {
        ...
        if (header.sequence != sequence) return error.WrongSequence;
...
pub fn connectUnix(allocator: std.mem.Allocator, io: std.Io, address: std.Io.net.UnixAddress, options: OpenOptions) !@This() {
...
pub fn flush(self: *@This()) (std.Io.Writer.Error || std.Io.net.Stream.Writer.Error)!void {
    self.writer.interface.flush() catch |ierr| return if (self.writer.err) |err| err else ierr;
}

pub fn sendRequest(self: *@This(), header: Request.Header, value: anytype) !Request {
    var request: Request = .{ .connection = self };
    try request.writeHeader(header);
    try request.writeValue(value);
    try request.send();
    return request;
}
